...
## Objective
Classify EV charging sessions as Normal or Attack using the EV session features and the provided label (0 = benign, 1 = malicious). The classification tool should report top-k labels with confidence scores.
...
- Conduct F-test feature selection between features and labels.
...
- At inference, return the top-3 labels and confidence scores for each classifier.
...
## Evaluation Metrics
We evaluate IDS-Agent on both binary classification (benign vs. malicious) and multi-classification (benign plus specific attack categories).

Binary classification metrics:
- Accuracy: ratio of correctly predicted samples to total samples, measuring overall effectiveness.
- False Alarm Rate (FAR): proportion of false positives, i.e., benign flows incorrectly classified as malicious.
